...
class MolGXGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            model_file: name of the file containing the pickled model. Defaults to 'model.pkl'.
        ...
        self.model_filepath = os.path.join(resources_path, model_file)
        self.molgx = MolgxSdk()
        self.molecules_data, self.target_property = self.molgx.LoadPickle(
            self.model_filepath
        )
    ...
    @property
    def homo_energy_value(self) -> Tuple[float, float]:
        return self._homo_energy_value

    @homo_energy_value.setter
    def homo_energy_value(self, value: Tuple[float, float]) -> None:
        self._homo_energy_value = value
        self.parameters = self._create_parameters_dictionary()

    @property
    def lumo_energy_value(self) -> Tuple[float, float]:
        return self._lumo_energy_value

    @lumo_energy_value.setter
    def lumo_energy_value(self, value: Tuple[float, float]) -> None:
    ...
    def generate(self) -> List[str]:
        ...
        molecules_df = self.molgx.GenMols(self.molecules_data, self.parameters)
